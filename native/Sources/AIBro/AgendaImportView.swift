...
struct AgendaImportView:View {
    ...
    @ViewBuilder private var semesterPicker:some View {
            if format != "ics" {HStack{DatePicker(nativeUI("第一周周一", "Monday of week 1"),selection:$semester,displayedComponents:.date);Spacer();AgendaChoice(title:nativeUI("课表时区", "Timetable time zone"),value:$zone,options:TimeZone.knownTimeZoneIdentifiers.map{($0,$0)}).frame(width:200)}}
    }
