...
@main struct AgendaTests {
 static func main() throws {
  ...
  let west=AgendaImport.courses(csv,semester:try d("20260914T233000","America/Los_Angeles"),zone:"America/Los_Angeles")
  let westStart=try d("20260914T083000","America/Los_Angeles")
  check(west.events.count==4 && west.events[0].start == westStart,"western course zone retains semester date across host midnight")
  let tuesday=AgendaImport.courses(csv,semester:try d("20260915"),zone:"Asia/Shanghai")
  check(tuesday.events.isEmpty && tuesday.warnings.count==1,"non-Monday semester still rejected in course timezone")
